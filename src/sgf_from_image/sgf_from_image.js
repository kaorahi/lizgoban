...
function kth(k, z1, dz) {return z1 + dz * (k - 1)}

function draw_grids(x1, y1, mx, ny, dx, dy, style, g) {
    const [xm, yn] = [kth(mx, x1, dx), kth(ny, y1, dy)]
    const radius = Math.min(...[dx, dy].map(Math.abs)) * 0.25
    clear(g)
    g.strokeStyle = style; g.lineCap = 'square'
    g.lineWidth = radius * 2
    for (let k = 1; k <= mx; k++) {
        const xk = kth(k, x1, dx); line(g, xk, y1, xk, yn)
    }
    for (let k = 1; k <= ny; k++) {
        const yk = kth(k, y1, dy); line(g, x1, yk, xm, yk)
    }
}
...
function get_sgf() {
    ...
    const body = (color, prop) => {
        ...
        const sgfpos = (k, sign) => sgfpos_name[sign > 0 ? k - 1 : 19 - k]
...
window.ondrop = e => {
    ...
    for (let k = 0; k < data.length; k++) {
        const item = data[k], {kind, type} = item
        switch (kind) {
        case 'string':
