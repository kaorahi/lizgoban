...
const {start_fast_redo, stop_fast_redo, try_fast_redo} = require('./fast_redo.js')
...
const audio_cache = {}
ipc.on('play_sound', (e, file) => {
    const audio = audio_cache[file] || (audio_cache[file] = new Audio(file))
    audio.play()
})
...
function update_persona_code(given_code) {
    ...
    const valid = (code !== '')
    const html = persona_html_for_code(code)
    input.style.backgroundColor = valid ? '#fff' : '#fcc'
    input.value = code
    Q('#persona_info').innerHTML = html
    Q('#persona_info code').innerText = code
}
...
document.onkeydown = e => {
    ...
    const undo_or_redo = command =>
          ...
          e.repeat ? start_fast_redo(moves => main('busy', command, moves)) :
...
function update_button_etc(availability) {
    ...
    const pair_match_p = (R.in_pair_match === 'pair_match')
    const anomalous_pair_match_p = R.in_pair_match && !pair_match_p
    ...
    update_ui_element('.show_in_pair_match', pair_match_p)
