...
function get_default_mcts_note(id) {
    const state = mcts_state[id], {force_actual, target, humansl_profile} = state.mcts.params
    const common_str = "Generated by naive MCTS + KataGo's neural network."
    const maybe = (flag, str) => flag ? ' ' + str : ''
    const force_str = maybe(force_actual > 0 && !empty(R.future_moves),
                            `${Math.round(force_actual * 100)}% of visits are allocated to moves that were actually played in the game being analyzed.`)
    const target_str = maybe(target, `Winrate is replaced with the ownership at ${target}.`)
    const hum_str = maybe(humansl_profile, `Policy is the average of the original and HumanSL ${humansl_profile}'s.`)
    const names = `B: ${R.player_black || '???'}, W: ${R.player_white || '???'}`
    const game = M.get_game(), moves = `moves: ${game.movenum()} (${game.len() - game.init_len})`
    return `${names}, ${moves}\n${R.weight_info || ''}\n(${common_str}${force_str}${target_str}${hum_str})\n`
}
